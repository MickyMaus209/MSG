    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            if (args[0].equalsIgnoreCase("toggle")) {
                ...
                if (!playerData.isDeactivated()) {
                    ...
                    player.sendMessage(msg.getConfigData().getFormatedMessage("deactivated", player));
                } else {
                    playerData.setDeactivated(false);
                    player.sendMessage(msg.getConfigData().getFormatedMessage("activated", player));
                }
                return false;
                ...
                player.sendMessage(msg.getConfigData().getFormatedMessage("ignore_usage", player, "%command%", "/" + label + " " + args[0]));
                return false;
           /* } else if (args[0].equalsIgnoreCase("ignored")) {
                List<String> ignoredPlayers = playerData.getIgnoredPlayers();
                player.sendMessage(msg.getConfigData().getFormatedMessage("ignored_players", player, "%ignored_list%", ignoredPlayers));
                return false;
            }
            ...
            */
                ...
                player.sendMessage(msg.getConfigData().getFormatedMessage("spy_usage", player, "%command%", "/" + label + " " + args[0]));
                return false;
            }
        }
        ...
        if (args.length == 2 && (args[0].equalsIgnoreCase("spy"))) {
            ...
            if (args[1].equalsIgnoreCase("list")) {
                Set<Player> spyingPlayerList = spy.getSpyingPlayers();
                if (spyingPlayerList.isEmpty() && !spy.isSpyAll()) {
                    player.sendMessage(msg.getConfigData().getFormatedMessage("not_spying_anyone", player));
                    return false;
                }
                player.sendMessage(msg.getConfigData().getFormatedMessage("spying_list", player, "%spy_list%", spy.isSpyAll() ? "ALL" : spy.getSpyingPlayers().stream().map(Player::getName).collect(Collectors.toList())));
                return false;
            }
